    ...
    checkboxes.forEach(function (checkbox) {
        ...
        if (filterCheck && filterCheck.style.display === 'flex' && filterNumber) {
            ...
            itemTextes.forEach(function (itemText) {
                ...
                percentages[i].textContent = 40 + percentage + "%"; // NEW
                i = i + 1;
...
const div_favorit = document.getElementById('favorit')
...
function prevPage() {
    ...
    window.send(JSON.stringify({ 'message': "prev_page" }));
}
...
function nextPage() {
    ...
    window.send(JSON.stringify({ 'message': "next_page" }));
}
